...
1. Automatyczne generowanie fiszek:
   - Użytkownik (zalogowany lub niezalogowany) wkleja dowolny tekst (np. fragment podręcznika).
   - Aplikacja wysyła tekst do modelu LLM za pośrednictwem API.
   - Model LLM proponuje zestaw fiszek (pytania i odpowiedzi).
   - Dla zalogowanych użytkowników: Fiszki są przedstawiane w formie listy z możliwością akceptacji, edycji lub odrzucenia.
   - Dla niezalogowanych użytkowników: Fiszki są wyświetlane tylko do podglądu, bez możliwości zapisu lub akceptacji.
...
ID: US-010
Tytuł: Generowanie fiszek bez rejestracji (tryb demo)
Opis: Jako niezalogowany użytkownik chcę móc przetestować funkcję generowania fiszek przez AI bez konieczności rejestracji, aby ocenić wartość aplikacji przed założeniem konta.
Kryteria akceptacji:

- Niezalogowany użytkownik ma dostęp do widoku generowania fiszek z tym samym interfejsem co zalogowani użytkownicy.
- Po wklejeniu tekstu (od 1000 do 10 000 znaków) i kliknięciu przycisku generowania, aplikacja wysyła żądanie do API modelu LLM.
- Wygenerowane fiszki są wyświetlane tylko do podglądu w interfejsie.
- Brak możliwości akceptacji, edycji lub zapisania fiszek do bazy danych.
